  ...
  group('HighScorePage', () {
    ...
    testWidgets('renders highscore view', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(BlocProvider.value(
        value: toggleCubit,
        child: const MaterialApp(home: HighScorePage()),
      ));
    ...
    testWidgets('bottom nav bar has Local and Global',
      ...
      await tester.pumpWidget(BlocProvider.value(
        value: toggleCubit,
        child: MaterialApp(
            home: Scaffold(bottomNavigationBar: CustomBottomNavigationBar())),
      ));
      //finds one navigation bar
    ...
    testWidgets('bottom nav bar has current index = local',
        (WidgetTester tester) async {
      //if state is called return LOCAL
      when(() => toggleCubit.state).thenReturn(HSToggleCubit.LOCAL);
      // Build just the bottom nav bar
      ...
      Element candidate =
          find.byType(BottomNavigationBar, skipOffstage: true).evaluate().first;
      final castWidget = candidate.widget as BottomNavigationBar;
      expect(castWidget.items[castWidget.currentIndex].label, "Local");
    });

    testWidgets('bottom nav bar has current index = global',
        (WidgetTester tester) async {
    ...
    testWidgets('bottom nav has current index = LOCAL',
        (WidgetTester tester) async {
    ...
    testWidgets('verify tapping global=> toggle(GLOBAL)',
        (WidgetTester tester) async {
      //if state is called return false
      when(() => toggleCubit.state).thenReturn(HSToggleCubit.LOCAL);
      // Build just the bottom nav bar
    ...
    testWidgets('verify tapping global=> toggle(LOCAL) not called',
        (WidgetTester tester) async {
      //if state is called return false
      when(() => toggleCubit.state).thenReturn(HSToggleCubit.LOCAL);
      // Build just the bottom nav bar
    ...
    testWidgets('verify tapping local=> toggle(LOCAL) only once',
        (WidgetTester tester) async {
      //if state is called return false
      when(() => toggleCubit.state).thenReturn(HSToggleCubit.LOCAL);
      // Build just the bottom nav bar
    ...
    testWidgets('verify tapping local=> toggle(GLOBAL) NEVER',
        (WidgetTester tester) async {
      //if state is called return false
      when(() => toggleCubit.state).thenReturn(HSToggleCubit.LOCAL);
      // Build just the bottom nav bar
